...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 0)
				{
					...
					if (ImGui::SliderFloat("Plane in Focus", &planeInFocus, 0.0f, 1.0f))
					{
						temp->SetPlaneInFocus(planeInFocus);
					}
					if (ImGui::SliderFloat("Focal Length", &focalLength, 24.0f, 300.0f))
					{
						temp->SetFocalLength(focalLength);
					}
					if (ImGui::SliderFloat("Aperture", &aperture, 1.0f, 64.0f))
					{
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/PlayerShip8Bit.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(stoneMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 2.0f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, -45.0f);
			obj2.get<Transform>().SetLocalScale(glm::vec3(0.5));
		...
		std::vector<KeyPressWatcher> keyToggles;
		{
			...
			keyToggles.emplace_back(GLFW_KEY_1, [&]() {
				if (drawL) {
					...
					illuminationBuffer->SetLit(0);
					illuminationBuffer->SetAmb(1);
					illuminationBuffer->SetDif(1);
					illuminationBuffer->SetSpec(1);
					illuminationBuffer->SetTex(1);
					temp->SetIntensity(false);
				}
				else {
					drawL = true;

					illuminationBuffer->SetLit(1);
					illuminationBuffer->SetAmb(1);
					illuminationBuffer->SetDif(1);
					illuminationBuffer->SetSpec(1);
					illuminationBuffer->SetTex(1);
					temp->SetIntensity(true);
				}
			});
			...
			keyToggles.emplace_back(GLFW_KEY_2, [&]() {
				if (!drawAL) {
					...
					illuminationBuffer->SetLit(1);
					illuminationBuffer->SetAmb(1);
					illuminationBuffer->SetDif(0);
					illuminationBuffer->SetSpec(0);
					illuminationBuffer->SetTex(1);
					temp->SetIntensity(false);
				}
				else {
			...
			// Ambient + Specular + Depth of Field
			...
			keyToggles.emplace_back(GLFW_KEY_6, [&]() {
				if (toggleTextures) {
					...
					illuminationBuffer->SetDif(1);
					illuminationBuffer->SetSpec(1);
					illuminationBuffer->SetTex(0);

					skyboxMat->Shader->SetUniform("t", 0);
				}
				else {
					toggleTextures = true;
					...
					skyboxMat->Shader->SetUniform("t", 1);
				}
			});
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			{
				gBuffer->BindDepthAsTexture(30);

				effects[activeEffect]->ApplyEffect(illuminationBuffer);

				gBuffer->UnbindTexture(30);
